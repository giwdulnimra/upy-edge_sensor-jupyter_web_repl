...
// Methode: led.__del__()
static mp_obj_t core_led_del(mp_obj_t self_in) {
    core_led_obj_t *self = (core_led_obj_t*)MP_OBJ_TO_PTR(self_in);
    bridge_led_delete(self->led_wrapper);
    self->led_wrapper = NULL;
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_1(core_led_del_obj, core_led_del);
// deinit() alias for __del__()
static mp_obj_t core_led_deinit(mp_obj_t self_in) {
    return core_led_del(self_in);
}
static MP_DEFINE_CONST_FUN_OBJ_1(core_led_deinit_obj, core_led_deinit);
...
// method-table for corehead.LED-class
static const mp_rom_map_elem_t core_led_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR___del__), MP_ROM_PTR(&core_led_del_obj) },
    { MP_ROM_QSTR(MP_QSTR_deinit), MP_ROM_PTR(&core_led_deinit_obj) },
 ...
 */
// Initialization of corehead.Console object: __init__(self)
...
/*
 * include "led" and "console" classes into "corehead" module
 */
// set globals-table of corehead-module: contains class-objects
static const mp_rom_map_elem_t module_corehead_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_corehead) },
    { MP_ROM_QSTR(MP_QSTR_led), MP_ROM_PTR(&mp_type_Core_LED) },         // corehead.led
    { MP_ROM_QSTR(MP_QSTR_console), MP_ROM_PTR(&mp_type_Core_Console) }, // corehead.console
};
...
// register module for Python import: import corehead
